...
class TestVQDSolverFieldsUnit:
    ...
    def test_defaults(self):
        from qoro_maestro_pyscf import VQDSolver
        v = VQDSolver()
    ...
    def test_inner_solver_default(self):
        """Default inner solver is a MaestroSolver with defaults."""
        from qoro_maestro_pyscf import VQDSolver, MaestroSolver
        v = VQDSolver()
        assert isinstance(v.solver, MaestroSolver)
        assert v.solver.ansatz == "hardware_efficient"

    def test_inner_solver_custom(self):
        from qoro_maestro_pyscf import VQDSolver, MaestroSolver
        inner = MaestroSolver(ansatz="uccsd", backend="gpu", maxiter=500)
...
class TestPackageExportsVQD:
    ...
    def test_vqd_solver_in_all(self):
        ...
        assert "VQDSolver" in qoro_maestro_pyscf.__all__
    ...
    def test_compute_statevector_fidelity_in_all(self):
        import qoro_maestro_pyscf
        assert "compute_statevector_fidelity" in qoro_maestro_pyscf.__all__

    def test_compute_statevector_fidelity_importable(self):
        from qoro_maestro_pyscf import compute_statevector_fidelity
...
class TestNoGetStateVectorCalls:
    ...
    GUARDED_FILES = [
        "qoro_maestro_pyscf/maestro_solver.py",
        "qoro_maestro_pyscf/vqd_solver.py",
        "qoro_maestro_pyscf/expectation.py",
    ]
    ...
    def test_no_bare_get_state_vector_in_maestro_solver(self):
        """maestro_solver.py must not call maestro.get_state_vector directly."""
        raw_src = self._source("qoro_maestro_pyscf/maestro_solver.py")
        ...
        assert bare <= 0, (
            f"maestro.get_state_vector called directly in maestro_solver.py "
    ...
    def test_no_bare_get_state_vector_in_vqd_solver(self):
        ...
        src = self._code_only(self._source("qoro_maestro_pyscf/vqd_solver.py"))
        import re
        all_calls = re.findall(r'maestro\.get_state_vector\s*\(', src)
    ...
    def test_no_bare_get_state_vector_in_expectation(self):
        ...
        src = self._code_only(self._source("qoro_maestro_pyscf/expectation.py"))
        import re
        all_calls = re.findall(r'maestro\.get_state_vector\s*\(', src)
        assert len(all_calls) == 0, (
    ...
    def test_compute_overlap_exported(self):
        ...
        import qoro_maestro_pyscf
        assert "compute_overlap" in qoro_maestro_pyscf.__all__
    ...
    def test_compute_statevector_fidelity_works_without_get_state_vector(self):
        ...
        from maestro.circuits import QuantumCircuit
        from qoro_maestro_pyscf.backends import configure_backend
        from qoro_maestro_pyscf.expectation import compute_statevector_fidelity
